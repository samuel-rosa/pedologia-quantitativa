...
pacote [magmagrittragrittr]. Um _pipe_ estabelece uma comunicação direta entre dois procesos, representando 
...
direita para a esquerda ou então pensar de maneira reversa. É claro que isso é fácil de se fazer com o exemplo
acima. Contudo, à medida que aumenta a complexidade da sequência de operações e das funções utilizadas, mais
difícil fica de ler/decifrar um conjunto de funções aninhadas.

O operador `%>%` resolve todos esses problemas de uma vez só (seu atalho é `Ctrl + Shift + M`):
