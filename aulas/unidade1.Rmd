...
sp::spplot(
  ...
  lattice::xyplot(coord_y ~ coord_x, data = as.data.frame(perfis25@coords), pch = 21, col = 'red', lwd = 2) %>% 
...
sp::spplot(
  ...
  lattice::xyplot(coord_y ~ coord_x, data = pontos400@coords %>% as.data.frame(), pch = 21, col = 'red') %>% 
  latticeExtra::as.layer()
```
...
```{r}
pontos400$um <- sp::over(pontos400, pedologia25) %>% unlist()
pontos400@data %>% 
  filter(!is.na(um)) %>% 
  group_by(um) %>% 
  summarise(
    mean = mean(ca_kcl_aas),
    ci95 = 1.96 * sd(ca_kcl_aas) / sqrt(n()),
    n = n()) %>% 
  mutate(
    inf = round(mean - ci95, 2), 
    mean = round(mean, 2), 
    sup = round(mean + ci95, 2)) %>% 
...
sp::spplot(mapa_idw, 1, col.regions = col_soil_var, main = "Interpolação determinística") +
