...
Para verificar se você já tem o git instalado em seu computador, acesse o Terminal do RStudio (painel esquerdo inferior) e emita as duas linhas de comando abaixo:

    which git
    git --version

Se o git estiver instalado, o primeiro comando deverá produzir como saída o local onde os arquivos do git estão localizados -- por exemplo, `/usr/bin/git` --, enquanto o segundo comando mostra a versão do git que está instalada -- por exemplo, `git version 2.7.4`. Caso você não tenha o git instalado em seu computador, vá até o endereço abaixo e descarregue o instalador ou estudo as diretrizes para instalação para o seu sistema operacional.

* https://git-scm.com/downloads

Depois de instalado o git, precisamos fazer a sua configuração. Trata-se de informar seu nome e endereço de e-mail para o git. Isso porque o git registra essas duas informações à cada mudança feita no código fonte e enviada (`commit`) ao sistema de controle de versão. Isso é da maior importância quando duas ou mais pessoas colaboram no desenvolvimento do código fonte. Para configurar o seu nome e endereço de e-mail no git, acesse o Terminal no RStudio (painel esquerdo inferior) e emita as duas linhas de comando abaixo, informando os dados solicitados.
