...
Vamos ler os dados das imagens utilizando as funções `raster::raster()` e `raster::stack()`. Enquanto a 
primeira lê os dados de uma imagem, a segunda empilha as imagens -- o verbo da língua inglesa _to stack_
significa 'empilhar'. Durante o empilhamento, a função `raster::stack()` verifica se as imagens possuem as
mesmas dimensões e resolução espacial. No caso das bandas do sensor TM do Landsat 5, todas possuem as mesmas 
dimensões, ou seja, cerca de 170 km na direção norte-sul e 183 km na direção leste-oeste. Contudo, as bandas
possuem resolução espacial diferente. Especificamente, as bandas 1 a 6 e a banda 7 possuem resolução espacial
de 30 m, enquanto a banda 6 (termal) possui resolução espacial de 120 m. Assim, durante o empilhamento das 
bandas, reamostramos a banda 6 para a resolução de 30 m usando a função `raster::resample()`. Para não alterar
os valores de nível de cinza dos pixeis, utilizamos o método do vizinho mais próximo para a interpolação 
(`method = "ngb"`). Note que esse processamento demora algumas dezenas de segundos para executar. O resultado
é um objeto da classe `RasterStack`.
...
landsat <- 
  landsat %>% 
  raster::addLayer(
    raster::resample(x = raster::raster(files[6]), y = landsat, method = "ngb"), 
    raster::raster(x = files[7])
  )
landsat
```
...
```{r}
raster::plot(
  landsat, col = gray.colors(n = 256, start = 0, end = 1, gamma = 2.2, alpha = NULL), 
...
Uma das etapas mais importantes da utilizada de imagens de satélite para análises espaciais consiste na 
...
```{r}
par(mfrow = c(1, 2))
landsat[[3]] %>% raster::crop(y = ext) %>% hist(main = "Banda 3 original")
landsat[[3]] %>% raster::crop(y = ext) %>% raster::stretch() %>% hist(main = "Banda 3 transformada")
```
...
shv <- sapply(c(1:5, 7), function (i) {
  tmp <- landsat[[1]] %>% values() %>% table()
...
# we avoid zero because this is the value of the borders and could be an optimiztic estimate.
