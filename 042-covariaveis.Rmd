...
# raster::plotRGB(landsat_crop, r = 3, g = 2, b = 1)
```

```{r}
library(mapedit)
tmp <- landsat %>% raster::crop(ext) %>% mapview::viewRGB(r = 5, g = 4, b = 3)
a <- drawFeatures(tmp)
plot(a)
```
...
# rgb432 <- mapview::viewRGB(landsat_crop, r = 4, g = 3, b = 2)
rgb432 <- landsat %>% raster::crop(ext) %>% mapview::viewRGB(r = 4, g = 3, b = 2)
floresta <- mapedit::drawFeatures(rgb432)
...
```{r}
dos <- 
  lapply(c(1:5, 7), function (i) {
    i <- 1
    landsat::DOS(
      sat = 5, SHV = min(dos_param$shv), SHV.band = 1, Grescale = dos_param$gain[i], 
      Brescale = dos_param$bias[i], edist = dos_param$edist[i], sunelev = dos_param$sunelev[i])$DNfinal.mean
  })
```
