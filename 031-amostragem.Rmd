<!-- # PARTE III -- AMOSTRAGEM ESPACIAL {-} -->

<!-- # Princípios da Amostragem Espacial -->

# Amostragem Probabilística

**Atenção!** Este capítulo está em fase de *desenvolvimento*. Visite a [página do projeto][issues] se tiver 
alguma sugestão, dúvida ou estiver disposto a colaborar. Sua opinião/ajuda é muito importante!

## Introdução

A característica fundamental da amostragem probabilística é que a chance de um determinado local ser amostrado
(probabilidade de inclusão) é conhecida e maior do que zero. Em outras palavras, todo e qualquer local possui 
alguma chance de ser amostrado, mesmo que alguns tenham maior chance do que outros. Um local que não pode ser 
amostrado tem probabilidade de inclusão igual a zero.

A amostragem probabilística é muito utilizada em experimentos controlados, como aqueles desenvolvidos em 
campos experimentais, casas de vegetação e laboratórios. No caso da modelagem espacial do solo, a amostragem
probabilística costuma ser usada para a validação das predições espaciais. Entretanto, ela também pode ser 
usada para obter observações para a calibração dos modelos preditivos.

```{r, meuseRiverPlot}
data(meuse.grid, package = "sp")
sp::gridded(meuse.grid) <- ~ x + y
sp::proj4string(meuse.grid) <- sp::CRS("+init=epsg:28992")
data(meuse.riv, package = "sp")
plot(
  meuse.grid@coords, type = "n", asp = 1,
  xlab = "Longitude (m)", ylab = "Latitude (m)", 
  main = "Conjunto de dados do Rio Meuse")
polygon(meuse.riv, col = "lightblue", border = "lightgray")
image(
  meuse.grid, "soil", col = terrain.colors(3), axes = TRUE, add = TRUE)
legend(
  "topleft", title = "Tipo de solo", fill = terrain.colors(3),
  legend = c("Solo calcário", "Solo não-calcário", "Solo vermelho"),
  border = terrain.colors(3), bty = "n")
text(179050, 331200, "Rio Meuse", srt = 50, cex = 0.7)
```

## Amostragem aleatória simples

Inglês: *simple random sampling*.

```{r, eval=FALSE}
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "random"), pch = 21, cex = 0.75)
```

```{r, echo=FALSE}
<<meuseRiverPlot>>
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "random"), pch = 21, cex = 0.75)
legend(176100, 332750, legend = "Amostras", pch = 21, bty = "n")
```

## Amostragem aleatória estratificada simples

Inglês: *stratified simple random sampling*.

```{r, eval=FALSE}
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "stratified"), pch = 21, cex = 0.75)
```

```{r, echo=FALSE}
<<meuseRiverPlot>>
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "stratified"), pch = 21, cex = 0.75)
legend(176100, 332750, legend = "Amostras", pch = 21, bty = "n")
```

## Amostragem aleatória sistemática alinhada

Inglês: *systematic random sampling*, *centric systematic random sampling*, 
*aligned systematic random sampling*.

```{r, eval=FALSE}
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "regular"), pch = 21, cex = 0.75)
set.seed(1984)
points(sp::spsample(meuse.grid, n = 15, type = "regular"), pch = 20, cex = 0.75)
```

```{r, echo=FALSE}
<<meuseRiverPlot>>
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "regular"), pch = 21, cex = 0.75)
set.seed(1984)
points(sp::spsample(meuse.grid, n = 15, type = "regular"), pch = 20, cex = 0.75)
legend(176100, 332750, legend = "Amostras (2001)", pch = 21, bty = "n")
legend(176100, 332400, legend = "Amostras (1984)", pch = 20, bty = "n")
```

## Amostragem aleatória sistemática desalinhada

Inglês: *nonaligned systematic random sampling*.

```{r, eval=FALSE}
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "nonaligned"), pch = 21, cex = 0.75)
```

```{r, echo=FALSE}
<<meuseRiverPlot>>
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "nonaligned"), pch = 21, cex = 0.75)
legend(176100, 332750, legend = "Amostras", pch = 21, bty = "n")
```

## Amostragem aleatória agrupada

Inglês: *clustered random sampling*.

```{r, eval=FALSE}
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "clustered", nclusters = 6), pch = 21, cex = 0.75)
```

```{r, echo=FALSE}
<<meuseRiverPlot>>
set.seed(2001)
points(sp::spsample(meuse.grid, n = 15, type = "clustered", nclusters = 6), pch = 21, cex = 0.75)
legend(176100, 332750, legend = "Amostras", pch = 21, bty = "n")
```

<!-- # Amostragem Não-Probabilística -->
